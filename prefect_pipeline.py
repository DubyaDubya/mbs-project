...
from settings import security_issuance_src, security_issuance_dest, security_issuance_folder
...
def full_pipeline():
    ...
    security_issuance_migration(s3, security_issuance_src, security_issuance_dest)
    print(f"security data written to {security_issuance_dest}")
    loan_issuance_migration(s3, loan_issuance_src, loan_issuance_dest, loan_part)
    print(f"loan data written to {loan_issuance_dest}")
    ...
    loan_data_set = ds.dataset(loan_issuance_dest, partitioning=loan_part, filesystem=s3)
